...
function TrackThumb({ cover }: { cover: string | null | undefined }) {
  ...
  return (
    <div className="flex h-full w-full items-center justify-center" style={{ backgroundColor: "var(--fallback-album-bg, #f2f8fb)" }}>
      {cover && !failed ? (
        <img src={cover} alt="" className="h-full w-full object-cover" loading="lazy" draggable={false} onError={() => setFailed(true)} />
      ) : (
        <img src={kissLipsSrc()} alt="" className="h-full w-full max-h-[72%] object-contain opacity-90 p-[18%]" loading="lazy" draggable={false} />
      )}
    </div>
  );
}
